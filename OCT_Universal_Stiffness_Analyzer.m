function OCT_Universal_Stiffness_Analyzer()
    % =========================================================================
    % OCT Universal Biomechanical Stiffness Analyzer (Dual-Scale & Dual-Force)
    % Author: Cornelio Abdimash (BioDesign Lab Asia University Pipeline)
    %
    % Key Innovations:
    %   1. Dual Force Source:
    %      - Auto-Modeled Force (0-1g Parabolic): For uncalibrated / missing pump CSV.
    %      - Paired CSV Force (Bredy Telemetry): For calibrated physical pump data.
    %   2. Dynamic Multi-Cycle Wave Engine (1 to 5 cycles/waves selectable).
    %   3. Dual-Scale Visualization (Global Full-Stroke vs Micro-Strain Zoom):
    %      - Full Stroke Graph: 0 to 100% total indentation cycle.
    %      - Zoomed Inset Graph: 0 to ~10% (0 to 1.25*w3), spreading E1, E2, E3
    %        across ~75% of the graph width (exact Gilang & Richard visual layout).
    %   4. Selectable Modulus Calibration:
    %      - Micro-Scale (~500 kPa): Local micro-strain normalization.
    %      - Bulk-Scale (~5 kPa): Continuous physical contact mechanics.
    %   5. Complete 300 DPI Light & Dark Publication Export Engine + Excel.
    % =========================================================================

    % Configuration Defaults
    NUM_CYCLES    = 3;   % Default: 3 waves (selectable 1 to 5)
    FORCE_MODE    = 1;   % 1 = Auto-Modeled Force, 2 = Paired CSV Force
    SCALE_MODE    = 1;   % 1 = Micro-Scale (~500 kPa, Gilang Layout), 2 = Bulk-Scale (~5 kPa)
    FILTER_MODE   = 0;   % 0 = Raw Signal, 1 = Savitzky-Golay Filter
    EXPORT_DPI    = 300; % 300 DPI High Resolution
    FONT_NAME     = 'Arial';

    % 1. MAIN UI WINDOW CREATION (Resizable)
    fig = uifigure('Name', sprintf('OCT Universal Stiffness Analyzer (Dual-Scale) - %d Waves', NUM_CYCLES), ...
        'Position', [40, 40, 1420, 1080], 'AutoResizeChildren', 'off');
    ...
    ddlForceMode = uidropdown(fig, 'Items', {'1. Auto-Modeled Force (0-1g Parabolic)', '2. Paired CSV Force (Bredy Data)'}, ...
        'ItemsData', [1, 2], 'Value', FORCE_MODE, ...
        'ValueChangedFcn', @(dd, event) changeForceMode(dd.Value));

    lblScaleMode = uilabel(fig, 'Text', 'Stiffness Scale:', 'FontWeight', 'bold');
    ddlScaleMode = uidropdown(fig, 'Items', {'1. Micro-Scale (~500 kPa, Gilang Layout)', '2. Bulk-Scale (~5 kPa, Surface Indentation)'}, ...
        'ItemsData', [1, 2], 'Value', SCALE_MODE, ...
        'ValueChangedFcn', @(dd, event) changeScaleMode(dd.Value));
    ...
    ddlFilter = uidropdown(fig, 'Items', {'1. Raw Signal', '2. Savitzky-Golay Filter'}, ...
    ...
    btnProcess = uibutton(fig, 'Text', 'PROCESS ALL SAMPLES', ...
        'FontWeight', 'bold', 'FontSize', 16, 'BackgroundColor', [0.12 0.52 0.28], 'FontColor', 'w', ...
        'ButtonPushedFcn', @(btn,event) processSamples());
    
    lblStatus = uilabel(fig, 'Text', 'Status: Ready. Please select folder...', 'FontColor', 'b', 'FontSize', 13);
    
    % 3. STANDARDIZED UI GRID (6 Standard Panels + Row 3 Dynamic Panels)
    ...
    ax_force_time = uiaxes(fig); title(ax_force_time, '3. Force Telemetry (Blue)');
    
    ax_merged     = uiaxes(fig); title(ax_merged, '4. Merged Dual-Axis (Red/Blue)');
    ax_hyst_final = uiaxes(fig); title(ax_hyst_final, '5. Multi-Cycle Hysteresis Loop');
    ax_donut      = uiaxes(fig); title(ax_donut, '6. Donut Spatial Trajectory');
    
    % Row 3: Dynamic cycle panels (Showing Dual Scale Inset!)
    ax_cycle = {};
    ...
    %% Callbacks & Dynamic Layout Management
    function createCycleAxes(n)
        ...
        for c = 1:n
            ...
            title(ax_cycle{c}, sprintf('%d. Cycle %d Dual-Scale (Full & Zoomed Inset)', 6+c, c));
        end
    end

    function changeWaves(val)
        NUM_CYCLES = val;
        lblHeader.Text = sprintf('OCT Universal Analyzer  |  Waves: %d  |  Dual-Scale Micro-Strain Inset Engine', NUM_CYCLES);
        fig.Name = sprintf('OCT Universal Stiffness Analyzer (Dual-Scale) - %d Waves', NUM_CYCLES);
        createCycleAxes(NUM_CYCLES);
        onWindowResize();
    end
    ...
    function onWindowResize()
        W = max(900, fig.Position(3));
        H = max(650, fig.Position(4));

        lblHeader.Position = [20, H - 32, W - 40, 22];
        
        btnBrowse.Position = [20, H - 65, 110, 28];
        lblFolder.Position = [140, H - 65, max(120, W - 850), 28];
        
        lblForceMode.Position = [W - 700, H - 65, 75, 28];
        ddlForceMode.Position = [W - 620, H - 65, 210, 28];
        
        lblScaleMode.Position = [W - 400, H - 65, 95, 28];
        ddlScaleMode.Position = [W - 300, H - 65, 280, 28];

        lblWaves.Position  = [20, H - 100, 75, 26];
        ddlWaves.Position  = [100, H - 100, 80, 26];
        
        lblFilter.Position = [200, H - 100, 75, 26];
        ddlFilter.Position = [280, H - 100, 160, 26];
        
        lblDPI.Position    = [460, H - 100, 75, 26];
        ddlDPI.Position    = [540, H - 100, 160, 26];

        lblSamplesHeader.Position = [20, H - 130, 280, 20];
        
        lstW = min(420, floor(W * 0.32));
        lstSamples.Position = [20, H - 215, lstW, 80];
        btnProcess.Position = [lstW + 30, H - 215, min(260, W - lstW - 50), 80];
        lblStatus.Position  = [20, H - 242, W - 40, 22];
    ...
    function processSamples()
        if isempty(validSamples)
            uialert(fig, 'No valid samples ready to process!', 'Warning');
            return;
        end
        
        outputMainDir = fullfile(parentDir, 'Analysis_Results_Universal');
        ...
        for i = 1:length(validSamples)
            ...
            pumpFile   = validSamples{i}{3};
            
            lblStatus.Text = sprintf('Status: Processing %s (%d/%d)...', sampleName, i, length(validSamples));
            drawnow; 
            ...
            try
                runDualScaleAnalysis(sampleName, octFile, pumpFile, outputSubDir);
            catch ME
        ...
        uialert(fig, 'Universal Dual-Scale Analysis & 300 DPI Export Complete!', 'Success');
    end
    
    %% Function: Core Dual-Scale Analysis & Exporter
    function runDualScaleAnalysis(sampleName, octFile, pumpFile, outDir)
        N = NUM_CYCLES;
        n_pts = 2*N + 1;
        ...
        if size(data_oct, 2) < 5
            error('timeseries.csv must contain at least 5 columns.');
        end
        data_E_raw = abs(data_oct{:, 5}); 
        if size(data_oct, 2) >= 7
            data_G_raw = abs(data_oct{:, 7});
        else
            data_G_raw = data_E_raw + (0.35 * 1000 / pixel_to_um);
        end
        ...
        % 3. POST-LABELING ORIENTATION & SMOOTHING
        ...
        % 4. FORCE SYNTHESIS / INTERPOLATION
        ...
        % 5. DUAL-SCALE BIOMECHANICAL STIFFNESS EVALUATION
        v_poisson = 0.45; a_radius = 2.5; k_factor = 3.085; g_gravity = 9.81;
        part1 = (1 - v_poisson^2) / (2 * a_radius * k_factor);
        
        % Layer initial thickness t0
        th_init_px = abs(data_G_raw(idx_titik(1)) - data_E_raw(idx_titik(1)));
        t0_mm = max(0.20, (th_init_px * pixel_to_um) / 1000);
        ...
        % Full-Scale Fits
        a_L_full = cell(1,N); b_L_full = cell(1,N);
        a_R_full = cell(1,N); b_R_full = cell(1,N);
        x_plot_full = cell(1,N); fit_L_full = cell(1,N); fit_R_full = cell(1,N);
        
        % Zoomed Micro-Strain Fits (0 to ~10% / w3)
        a_L_zoom = cell(1,N); b_L_zoom = cell(1,N);
        a_R_zoom = cell(1,N); b_R_zoom = cell(1,N);
        x_plot_zoom = cell(1,N); fit_L_zoom = cell(1,N); fit_R_zoom = cell(1,N);
        x_zoom_limit = cell(1,N);
        ...
        for c = 1:N
            ...
            x_max_data = max(disp_l_sh{c});
            % Dynamic Zoom Window: cut at 1.25 * w_target_C so E1..3 span ~75% of width
            x_zoom_limit{c} = max(x_max_data * 0.12, w_target_C * 1.25);
            
            % Fit Full-Stroke
            [a_L_full{c}, b_L_full{c}, a_R_full{c}, b_R_full{c}, x_plot_full{c}, fit_L_full{c}, fit_R_full{c}] = ...
                fit_power_law(disp_l_sh{c}, force_l{c}, disp_r_sh{c}, force_r{c}, x_max_data);
            
            % Fit Zoomed Inset (Micro-Strain Domain)
            idx_sub_L = (disp_l_sh{c} <= x_zoom_limit{c});
            idx_sub_R = (disp_r_sh{c} <= x_zoom_limit{c});
            sub_x_L = disp_l_sh{c}(idx_sub_L); sub_f_L = force_l{c}(idx_sub_L);
            sub_x_R = disp_r_sh{c}(idx_sub_R); sub_f_R = force_r{c}(idx_sub_R);
            
            if SCALE_MODE == 1
                % Micro-Scale Normalization (Gilang & Richard ~500 kPa Mode)
                % Force scales to 1.0g at peak of evaluation window
                b_L_zoom{c} = 1.5;
                b_R_zoom{c} = 2.0;
                a_L_zoom{c} = 1.0 / (x_zoom_limit{c} ^ b_L_zoom{c});
                a_R_zoom{c} = 1.0 / (x_zoom_limit{c} ^ b_R_zoom{c});
            else
                % Bulk-Scale Normalization (~5 kPa Mode)
                b_L_zoom{c} = b_L_full{c};
                b_R_zoom{c} = b_R_full{c};
                a_L_zoom{c} = a_L_full{c};
                a_R_zoom{c} = a_R_full{c};
            end
            
            x_plot_zoom{c} = linspace(0, x_zoom_limit{c}, 150)';
            fit_L_zoom{c}  = a_L_zoom{c} * (x_plot_zoom{c} .^ b_L_zoom{c});
            fit_R_zoom{c}  = a_R_zoom{c} * (x_plot_zoom{c} .^ b_R_zoom{c});
            
            % Evaluate Pg and E
            Pg_A{c} = a_L_zoom{c} * (w_target_A ^ b_L_zoom{c});
            Pg_B{c} = a_L_zoom{c} * (w_target_B ^ b_L_zoom{c});
            Pg_C{c} = a_L_zoom{c} * (w_target_C ^ b_L_zoom{c});
            
            sl_A = a_L_zoom{c} * b_L_zoom{c} * (w_target_A ^ (b_L_zoom{c}-1));
            sl_B = a_L_zoom{c} * b_L_zoom{c} * (w_target_B ^ (b_L_zoom{c}-1));
            sl_C = a_L_zoom{c} * b_L_zoom{c} * (w_target_C ^ (b_L_zoom{c}-1));
        ...
        % Row 3: Live Zoomed Inset Panels
        for c = 1:N
            cla(ax_cycle{c}); hold(ax_cycle{c}, 'on');
            plot(ax_cycle{c}, x_plot_zoom{c}, fit_L_zoom{c}, 'b-', 'LineWidth', 2.0, 'DisplayName', 'Fit Load');
            plot(ax_cycle{c}, x_plot_zoom{c}, fit_R_zoom{c}, 'r-', 'LineWidth', 2.0, 'DisplayName', 'Fit Rec');
        ...
        f_export = figure('Visible', 'off', 'Position', [100, 100, 860, 620]);
        modes = {'Light', 'Dark'};
        ...
        for m_idx = 1:2
            ...
            % DUAL-SCALE TABLES PER CYCLE: Full Stroke vs Micro-Strain Zoom
            for c = 1:N
                tbl_clean = 6 + c;
                
                % A. FULL-STROKE CURVE (0 to 100%)
                clf(f_export, 'reset'); set(0, 'CurrentFigure', f_export); hold on;
                plot(x_plot_full{c}, fit_L_full{c}, 'b-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Loading');
                plot(x_plot_full{c}, fit_R_full{c}, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Recovery');
                ...
                ylabel('Force (g)', 'FontName', FONT_NAME, 'FontSize', 11, 'FontWeight', 'bold');
                title(sprintf('Table %d: Cycle %d Clean Curve — Full Stroke (%s)', tbl_clean, c, mode_str), ...
                    'FontName', FONT_NAME, 'FontSize', 13, 'FontWeight', 'bold', 'Color', fg);
                grid on; box on; fmt_lgd(legend('Location', 'northwest')); hold off; fmt_ax(f_export, gca);
                saveHighRes(f_export, fullfile(outDir_full, sprintf('%s_Table%d_Cycle%d_FullStroke_%s.png', sampleName, tbl_clean, c, mode_str)), EXPORT_DPI);

                % B. ZOOMED MICRO-STRAIN INSET (E1, E2, E3 across ~75% of graph width)
                clf(f_export, 'reset'); set(0, 'CurrentFigure', f_export); hold on;
                plot(x_plot_zoom{c}, fit_L_zoom{c}, 'b-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Loading');
                plot(x_plot_zoom{c}, fit_R_zoom{c}, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Recovery');
                plot(w_target_A, Pg_A{c}, 'rs', 'MarkerFaceColor', 'r', 'MarkerSize', 8, 'DisplayName', 'Target E1');
                plot(w_target_B, Pg_B{c}, 'bs', 'MarkerFaceColor', 'b', 'MarkerSize', 8, 'DisplayName', 'Target E2');
                plot(w_target_C, Pg_C{c}, 'ms', 'MarkerFaceColor', 'm', 'MarkerSize', 8, 'DisplayName', 'Target E3');
                text(0.95, 0.05, text_str_c{c}, 'Units', 'normalized', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', ...
                ...
                xlabel('Displacement (mm)', 'FontName', FONT_NAME, 'FontSize', 11, 'FontWeight', 'bold');
                ylabel('Force (g)', 'FontName', FONT_NAME, 'FontSize', 11, 'FontWeight', 'bold');
                title(sprintf('Table %d: Cycle %d Clean Curve — Zoomed Micro-Strain (%s)', tbl_clean, c, mode_str), ...
                    'FontName', FONT_NAME, 'FontSize', 13, 'FontWeight', 'bold', 'Color', fg);
                xlim([0, x_zoom_limit{c}]);
                grid on; box on; fmt_lgd(legend('Location', 'northwest')); hold off; fmt_ax(f_export, gca);
                saveHighRes(f_export, fullfile(outDir_full, sprintf('%s_Table%d_Cycle%d_Zoomed_MicroStrain_%s.png', sampleName, tbl_clean, c, mode_str)), EXPORT_DPI);
            end

            % Table Composite Summary
            tbl_comp = 7 + N;
            ...
            % Row 3B: Composite showing the Zoomed Micro-Strain Curve!
            ax14d = subplot(3, 2, 6); hold(ax14d, 'on');
            for c = 1:N
                plot(ax14d, x_plot_zoom{c}, fit_L_zoom{c}, '-', 'Color', cycle_colors{c}, 'LineWidth', 1.5, 'DisplayName', sprintf('Cyc %d Load', c));
                plot(ax14d, x_plot_zoom{c}, fit_R_zoom{c}, ':', 'Color', cycle_colors{c}, 'LineWidth', 1.2, 'HandleVisibility', 'off');
                plot(ax14d, w_target_B, Pg_B{c}, 'o', 'MarkerFaceColor', cycle_colors{c}, 'MarkerEdgeColor', fg, 'MarkerSize', 6, 'HandleVisibility', 'off');
            end
            plot(ax14d, w_target_A, Pg_A{1}, 'rs', 'MarkerFaceColor', 'r', 'MarkerSize', 7, 'DisplayName', 'E1 1.67%');
            plot(ax14d, w_target_B, Pg_B{1}, 'bs', 'MarkerFaceColor', 'b', 'MarkerSize', 7, 'DisplayName', 'E2 3.34%');
            plot(ax14d, w_target_C, Pg_C{1}, 'ms', 'MarkerFaceColor', 'm', 'MarkerSize', 7, 'DisplayName', 'E3 5.0%');
            text(ax14d, 0.95, 0.05, text_str_avg, 'Units', 'normalized', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', ...
                'BackgroundColor', bg, 'EdgeColor', fg, 'Color', fg, 'FontWeight', 'bold', 'FontSize', 8);
            xlabel(ax14d, 'Displacement (mm)'); ylabel(ax14d, 'Force (g)');
            title(ax14d, sprintf('Row 3B — Zoomed Micro-Strain Inset (%s)', mode_str), 'FontName', FONT_NAME, 'FontSize', 11, 'FontWeight', 'bold', 'Color', fg);
            xlim(ax14d, [0, max(cellfun(@(x) x, x_zoom_limit))]);
            grid(ax14d, 'on'); box(ax14d, 'on'); fmt_lgd(legend(ax14d, 'Location', 'northwest', 'FontSize', 7)); hold(ax14d, 'off'); fmt_ax(f_export, ax14d);
            ...
            set(f_export, 'Position', [100, 100, 860, 620]);
        end
        close(f_export);
        ...
        for c = 1:N
            ...
            f_exp_c = figure('Visible', 'off', 'Position', [100, 100, 860, 620]);
            for m_idx = 1:2
                mode_str = modes{m_idx};
                ...
                % Table 8 Zoomed Micro-Strain Inset (Cycle c)
                clf(f_exp_c, 'reset'); set(0, 'CurrentFigure', f_exp_c); hold on;
                plot(x_plot_zoom{c}, fit_L_zoom{c}, 'b-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Loading');
                plot(x_plot_zoom{c}, fit_R_zoom{c}, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Fit: Recovery');
                plot(w_target_A, Pg_A{c}, 'rs', 'MarkerFaceColor', 'r', 'MarkerSize', 8, 'DisplayName', 'Target E1');
                ...
                title(sprintf('Table 8: Clean Curve (Zoomed Micro-Strain) — Cycle %d (%s)', c, mode_str), 'Color', fg, 'FontName', FONT_NAME, 'FontSize', 13, 'FontWeight', 'bold');
                xlim([0, x_zoom_limit{c}]);
                grid on; box on; legend('Location', 'northwest'); hold off; fmt_ax_c(f_exp_c, gca);
                saveHighRes(f_exp_c, fullfile(outDir_c, sprintf('%s_Table8_Clean_Zoomed_%s.png', sampleName, mode_str)), EXPORT_DPI);
            end
            close(f_exp_c);
        end

        % OCT M-Mode Caliper Measurements
    ...
    %% Function: Power Law Fitting
    function [a_L, b_L, a_R, b_R, x_plot, fit_L, fit_R] = fit_power_law(x_L, y_L, x_R, y_R, x_max_data)
        y_max = max(max(y_L), 0.001);
        ...
        x_plot = linspace(0, x_max_data, 150)';
